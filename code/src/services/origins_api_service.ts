...
class OriginsApiService {
	// préfixe de l'API
	private prefix = "/api/origins";

	// sélection de tous les enregistrements
	public selectAll = async (): Promise<ApiResponse<Origins[]>> => {
	...
	public selectOne = async (id: number): Promise<ApiResponse<Origins>> => {
		// configurer la requête HTTP
		const request = new Request(`http://127.0.0.1:3000${this.prefix}/${id}`);
	...
	public insert = async (data: FormData): Promise<ApiResponse<Origins>> => {
		// configurer la requête HTTP
		const request = new Request(`http://127.0.0.1:3000${this.prefix}`, {
			method: "post",
...
export default OriginsApiService;
