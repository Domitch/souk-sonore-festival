    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          >
            <span className="text-2xl tracking-tight text-[#d4a574]">Souk Sonore</span>
            <span className="text-xs text-[#b8a99a] tracking-widest uppercase">Festival 2025</span>
          </motion.button>
            ...
            {menuItems.map((item) => (
              <motion.button
                ...
                className={`relative px-3 py-2 transition-colors ${
                  currentPage === item.id ? 'text-[#d4a574]' : 'text-[#f5f1ea] hover:text-[#d4a574]'
                }`}
                whileHover={{ y: -2 }}
              >
                {item.label}
